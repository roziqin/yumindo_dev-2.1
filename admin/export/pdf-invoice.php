...
$data3=mysqli_fetch_assoc($query3);
  ...
  $ppnstat = $data['pengukuran_ppn'];
  $ppn = $data['pengukuran_ppn_jumlah'];
  $status = $data['pengukuran_status'];
...
$html = '
    ...
    if ($ppnstat=='ya') {
        $ketppn = '<tr>
                    <td colspan="5">PPN</td>
                    <td style="text-align:right;">Rp '.format_rupiah($ppn).'</td>
                  </tr>';
    } else {
        $ketppn = '';
    }
    if ($status=="Lunas") {
      $html.='
              ...
              <tr>
                  ...
                  <td style="text-align:right;"><b>Lunas</b></td>
              </tr>
          </table>
...
$dompdf->stream("forminvoice-".$tgl."-".$pelanggan.".pdf", array("Attachment"=>0));
